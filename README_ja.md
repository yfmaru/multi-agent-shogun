...
Phase 2はClaude Codeエージェントのbusy/idleを**二枚の印方式**（cmd_217）で判定する——`/tmp/shogun_busy_{agent}`と`/tmp/shogun_idle_{agent}`という2つのタイムスタンプファイルをmtimeで比較し、[`lib/agent_status.sh`](lib/agent_status.sh)の`agent_turn_state()`がbusy印のmtimeがidle印のmtime**以上**ならbusy、さもなくば（busy印が未だ存在せぬ場合を含め）idleと返す（同秒同着はbusy側に倒す——安全側）。これは、単一のidleフラグを使う旧方式——一度でも生まれたフラグが消えず、`claude`向け`agent_is_busy()`が実質常にidleを返し続けてしまっていた（フラグの**中身**をtouchしても、存在の有無しか見ない判定は1ミリも変わらなかった）——を置き換えたものである。二枚の印方式はCLI自身が申告するターン境界を読む: busy印は`UserPromptSubmit` hook（[`scripts/user_prompt_submit_hook.sh`](scripts/user_prompt_submit_hook.sh)、ターン開始時）・`session_start_hook.sh`（**`SessionStart`の`source`が`clear`または`compact`の時に限る**——`startup`/`resume`は走っているセッションへの割込みではなくプロンプト静止状態であり、ここでbusy印を押すと最大27分38秒idleのままbusy扱いされ続ける偽busyを生んだ実例がある——cmd_230, FAKE_BUSY_POST_RESTART_FIX）・`stop_hook_inbox.sh`のblock経路（ターン継続時）・`scripts/post_tool_use_hook.sh`（ターン中の道具呼び出しのたび、後述）がtouchし、idle印は`stop_hook_inbox.sh`の通常終了経路（shogunの早期return経路を含む——それ以外にidle印の書き手が構造上無いため）と`inbox_watcher.sh`起動時がtouchする。

`user_prompt_submit_hook.sh`は自身のagent識別を`TMUX_PANE`一本足に頼らず、三段の梯子で解決する——`TMUX_PANE`はこのhookイベントに限って一部セッションで消える実例が観測されており（原因未解明）、単独依存を避けるためである: **L1** `TMUX_PANE` → `tmux display-message`（最も安い・最初に試す）。**L2** プロセス系譜を遡り、各tmux paneの`pane_pid`と照合する（`/proc/<pid>/status`の`PPid`フィールドを使う——`/proc/<pid>/stat`第4フィールドの近道はプロセス名に空白や括弧が含まれるとフィールド番号がずれるため使わない）。**L3** hookのstdin JSONから`session_id`を読み、`session_start_hook.sh`が`/tmp/shogun_session_{session_id}`へ書いた`session_id→agent_id`対応表を引く。どの段が失敗しても次の段へ落ちるだけで、三段すべて空振りした場合はログを1行残してexit 0する——非0で終わることは決してない（hookの失敗は全軍のプロンプト投入を同時に壊すため）。

`UserPromptSubmit`が実際に装填されているかは、そのhookだけがtouchする専用の印`/tmp/shogun_ups_{agent}`で判定する——共有のbusy印ではない（busy印は`session_start_hook.sh`が`/clear`のたびにも touch するため、mtimeが進んだだけでは何が動かしたのか復元できない）。`inbox_watcher.sh`の`check_hook_armed()`はこのups印のmtimeを「1回前のnudge時点」のスナップショットと比較する（hookはsend-keysのおよそ1秒後に発火するため、送出直後の同期チェックでは必ず取り逃す）。nudgeをN=3回送ってもups印のmtimeが一度も前進しなければ`[HOOK-UNARMED]`を一度だけログ・通知し、前進すれば`[HOOK-ARMED]`を一度だけログする。`user_prompt_submit_hook.sh`自身も成功経路に1行ログを残すようになった——従来は失敗経路にしかログが無く、「ログが増えぬ」が「hookは健全」とも「hookは一度も発火していない」とも読めてしまっていた。

stale-busy安全網（恒久busyの上限の一つ）は、齢が300秒に達すること**かつ**その後**画面が同じ300秒の窓の間ずっと停止していること**の両方を要求する（齢だけではない）。画面停止の計測窓は齢が300秒に達して初めて標本を採り始めるため、**実効の合計待ち時間は300秒ではなく概ね640秒**である（実測4件が裏付ける: 641s / 626s / 645s / 619s——cmd_230, `queue/reports/gunshi_report.yaml` finding_3）。これはstall検知の`is_stalled_pane()`と同じhash比較の仕組みを共有するが（画面が動いていれば生きている、という同じ発想）、状態も閾値も別物であり（`is_stalled_pane()`の`stall_after_sec`とは混ぜない）専用に持つ。従来は齢だけで強制idle化するに足りたため、本物の長時間ターン（画面は動き続けている）が、二枚の印方式そのものが防ごうとした「繁忙なのにidleと誤判定される」失敗形に落ちることがあった——本機能の設計レビュー最中に、まさにこの現象がある足軽へ実際に起きている。pane captureが失敗または空を返した場合は判定不能として扱い、強制idle化は**行わない**。この網は未読メッセージが1件以上あることを要件とする（`FIRST_UNREAD_SEEN > 0`）——未読0の間はこの網が原理的に発火し得ない（cmd_243 §0-D）。以下の第三の網はこの穴を塞ぐために追加された。

**cmd_243 — 未読非依存の第三の網**（`scripts/inbox_watcher.sh`の`stale_busy_mark_net()`。`liveness_tick()`から呼ぶ）は`FIRST_UNREAD_SEEN`を代理指標に使わず、busy印そのもののmtimeを直接読む。ゆえに`liveness_tick()`がstall検知に既に与えている壁時計と同じ周期で発火し、未読が一度も来ぬ場合でも動く。上記の網と同じ二段構えのゲート（印の齢>=300秒**かつ**画面停止>=300秒）を保ち、加えて`pane_input_safety()`を確認する——画面が`modal`（実際に入力を待つ画面）と判定されれば、強制idle化はせず通知のみを出す。ゆえに本物の入力待ちターンを断ち切ることはない。ログ行は`[STALE-BUSY-MARK]`のタグを持ち、legacy網の`stale busy recovery`行とは区別される（legacy網は撤去せず並存し、それぞれ独自の一回性通知ガードを持つ）。本網がstall検知自身のエスカレーション（既定`stall_after_sec`=900秒、より長い）を最大約300秒先回りして印を倒し得るが、これは許容している——その分だけ本網自身の通知が同じ事実をより早く届けるためである。

表示層にも対をなす是正がある（cmd_243 §B）。`lib/agent_status.sh`の`get_pane_busy_rc()`は従来、`claude`型エージェントのturn状態印がidleを返すたび——上記いずれかの安全網がbusy印を凍結して強制idle化した直後を含め——pane解析（`agent_is_busy_check()`）へ落ちていた。stale-busy網が発火した直後の画面には、まだ最後の表示行が残っている（多くの場合、文字どおり`esc to interrupt`を含む——「まだ働いている」ように見える画面こそが凍結の正体だからである）ため、印は正しくidleへ転じたのに表示だけがbusyを言い続けていた——印とそれを支える表示のうち、片方しか直っていなかったことになる。`get_pane_busy_rc()`は現在、hookが書いた印が一つも無い場合（`/tmp/shogun_ups_{agent}`・`/tmp/shogun_busy_{agent}`のいずれも不在）に限ってpane解析へ落ちる——これは上記`[HOOK-UNARMED]`が検知するmtimeベースの条件と関連はするが、同一ではない。いずれかの印が存在する場合、idleのturn状態はそのまま信用する。

matcher無し（全ツール対象）の`PostToolUse` hook（`scripts/post_tool_use_hook.sh`）が、ターンの最中にも道具呼び出しのたびbusy印を再touchするようになった——従来はターンの開始と終了の間を進める者が一人も居らず、stale-busy安全網の実効窓（概ね640秒）を超えるターンは上記の画面停止チェックの有無にかかわらずいずれ安全網に触れてしまっていた。これは画面停止チェックの補強であり代替ではない: stale-busy安全網は本hookが未装填でも正しく機能するよう作られている。**`.claude/settings.json`への新規hook登録は、稼働中のCLIセッションへは届かない**——そのセッションのプロセスが再起動されて初めて有効になる。

モーダル表示中のpaneへ送出してはならない経路は、これとは独立に各々のゲート（[`pane_has_open_modal()`](lib/agent_status.sh)）を持つ——hook未装填の日にも効く第二の網である。

**pane幅の既知の制約**: pane解析ベースのbusy検出（`agent_is_busy_check()`。非`claude` CLI向けおよびstall検知が使用）はtmux pane幅44で不正確になる——Claude Codeのステータスバーのヒント行が`esc to interrupt`部分文字列が現れる前に切り詰められるため、繁忙時にもidleと誤判定し得る（2026-08-09実測。本リポジトリの既定布陣の約2/3が該当）。上記の二枚の印方式はpaneテキストを一切参照せぬためこの影響を受けない。pane幅自体の是正は別途扱う。
...
メッセージ配送とは別に、エージェントが完全に沈黙した状態を検知する層がある——タスクを保持したままターンの途中で止まっている状態は「考え中」と見分けがつかないため、「未読メッセージが無い」だけでは判定できない:

- **`baton_watchdog.sh`** — エージェントごとではなく**単一インスタンスのグローバルwatcher**。約60秒周期で`queue/`配下のYAMLを読み、独立した4種の停止パターンを判定する: 未完了のcmdが残っているのに誰もタスクを保持していない（「バトン喪失」）、配送機構自体が死んでいる、エージェントがタスクを保持したまま長時間無進捗、エージェントのinboxがwatcherは生きているのに読まれず滞留している。停止検知については**読み取り専用の診断のみ**を行う——tmuxへのキー送信・プロセス終了・タスクの再割当は一切行わず、通知（他エージェントのinboxへの書き込み、長時間の停止では主のスマホへのntfy）を書くのみである。ただし1点例外がある。同じスクリプトは定期的な文脈リセット（`periodic_clear_enabled`、既定は無効・主の裁可により有効化）も実装しており、karo・軍師が手隙の時に`clear_command`を書き込む。これは`inbox_watcher.sh`により`/clear`のキー送信として当人へ届く。主のご判断待ちのcmdは`queue/shogun_to_karo.yaml`で印を付けられ、これがあると「バトン喪失」は約900秒ごとには急かさなくなる——ただし印は検知を黙らせるのではなく急かす間隔を延ばすだけであり、印が付いたまま外し忘れても`baton_lost_human_held_after_sec`（既定86400秒＝24時間）の安全網が、開いている印のうち最も古い`awaiting_since`から計った経過時間で必ず発火する。**印は`awaiting: lord`と`awaiting_since: "<ISO8601形式の時刻>"`の両方を対で書くこと。** `awaiting: lord`のみを書くと、番犬プロセスが検知した瞬間を起点に計時する旧形式フォールバックへ落ち、番犬プロセスの再起動をまたぐと計時が巻き戻る——安全網が本来防ぐべきその不具合（OBS-61-1）そのものに陥る。
- **`watcher_supervisor.sh`** — 全`inbox_watcher.sh`インスタンスと`baton_watchdog.sh`を常時監視し、死んでいれば再起動する軽量ループ。多重起動は各段で`pgrep`により防止される（supervisor自身・各watcher・番犬のいずれも）。

**コード変更後の入れ替えについて:** これらは常駐プロセスであり、コードを更新しても走っているデーモン自体は自動では入れ替わらない。新コードへ切り替えるには、走っているプロセスを一度停止し（エージェント自身は行わない——`CLAUDE.md`のD006参照）、supervisorに再起動させる必要がある（数秒で完了する）。本プロジェクトでは「マージされた」ことを「新コードで動いている」ことの証明とせず、常にデーモンの**起動時刻**とコードの**マージ時刻**を突き合わせて確認する運用を徹底している。
