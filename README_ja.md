...
300秒のstale-busy安全網（恒久busyの唯一の上限）は、齢だけでなく**画面が同じ窓の間ずっと停止していること**も併せて要求するようになった。これはstall検知の`is_stalled_pane()`と同じhash比較の仕組みを共有するが（画面が動いていれば生きている、という同じ発想）、状態も閾値も別物であり（`is_stalled_pane()`の`stall_after_sec`とは混ぜない）専用に持つ。従来は齢だけで強制idle化するに足りたため、本物の長時間ターン（画面は動き続けている）が、二枚の印方式そのものが防ごうとした「繁忙なのにidleと誤判定される」失敗形に落ちることがあった——本機能の設計レビュー最中に、まさにこの現象がある足軽へ実際に起きている。pane captureが失敗または空を返した場合は判定不能として扱い、強制idle化は**行わない**。

matcher無し（全ツール対象）の`PostToolUse` hook（`scripts/post_tool_use_hook.sh`）が、ターンの最中にも道具呼び出しのたびbusy印を再touchするようになった——従来はターンの開始と終了の間を進める者が一人も居らず、300秒を超えるターンは上記の画面停止チェックの有無にかかわらずいずれstale-busy安全網に触れてしまっていた。これは画面停止チェックの補強であり代替ではない: stale-busy安全網は本hookが未装填でも正しく機能するよう作られている。**`.claude/settings.json`への新規hook登録は、稼働中のCLIセッションへは届かない**——そのセッションのプロセスが再起動されて初めて有効になる。
