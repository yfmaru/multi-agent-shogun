...
Phase 2 distinguishes busy vs idle for Claude Code agents with a **two-marker design** (cmd_217): two timestamp files, `/tmp/shogun_busy_{agent}` and `/tmp/shogun_idle_{agent}`, are compared by mtime — `agent_turn_state()` in [`lib/agent_status.sh`](lib/agent_status.sh) reports `busy` when the busy marker's mtime is `>=` the idle marker's (ties go to busy — the safe side), and `idle` otherwise, including when the busy marker doesn't exist yet. This replaced an earlier presence-only design where a single idle flag, once created, was never deleted mid-session — `agent_is_busy()` for `claude` ended up reporting idle almost permanently regardless of actual pane state, because touching the flag's *contents* couldn't change a check that only asked "does it exist." The two-marker design instead reads CLI-reported turn boundaries: the busy marker is touched by the `UserPromptSubmit` hook ([`scripts/user_prompt_submit_hook.sh`](scripts/user_prompt_submit_hook.sh)) when a turn starts, by `session_start_hook.sh` **only when `SessionStart`'s `source` is `clear` or `compact`** (a fresh `startup`/`resume` session sits at the prompt with no turn running, so touching busy there produced a false-busy state observed live for up to 27m38s post-restart — cmd_230, FAKE_BUSY_POST_RESTART_FIX), by `stop_hook_inbox.sh`'s block path when a turn continues, and by `scripts/post_tool_use_hook.sh` on every tool call mid-turn (see below); the idle marker is touched by `stop_hook_inbox.sh`'s normal exit path (including the `shogun` early-return path, which otherwise has no idle-marker writer at all) and by `inbox_watcher.sh` at startup.

`user_prompt_submit_hook.sh` resolves its own agent identity through a three-rung ladder instead of depending solely on `TMUX_PANE` — which was observed going missing for this specific hook event on some sessions, cause unresolved: **L1** `TMUX_PANE` → `tmux display-message` (cheapest, tried first); **L2** process ancestry walked up to a pane's `pane_pid`, matched via `/proc/<pid>/status`'s `PPid` field (not the `/proc/<pid>/stat` field-4 shortcut, which shifts when a process name contains spaces or parentheses); **L3** a `session_id → agent_id` lookup table that `session_start_hook.sh` writes to `/tmp/shogun_session_{session_id}` from the hook's stdin JSON. Any rung failing simply falls through to the next; if all three miss, the hook logs one line and exits 0 — never non-zero, since a hook failure would break prompt submission fleet-wide.

Whether `UserPromptSubmit` is actually armed is checked against a marker that hook alone ever touches — `/tmp/shogun_ups_{agent}` — rather than the shared busy marker, which `session_start_hook.sh` also touches on every `/clear` (making busy-marker mtime progress alone an unreliable signal of what actually moved it). `inbox_watcher.sh`'s `check_hook_armed()` compares this `ups` marker's mtime against a snapshot taken one nudge earlier — the hook fires roughly a second after send-keys, so checking synchronously in the same call that just sent the nudge would always miss it — then logs `[HOOK-UNARMED]` (once) and notifies once `N=3` nudges pass with no advance ever observed, or `[HOOK-ARMED]` (once) the moment one is. `user_prompt_submit_hook.sh` also now logs one line on its success path, not only on failure — previously "no new log lines" was ambiguous between "hook is healthy" and "hook never fires."

The stale-busy safety net — one of the upper bounds on permanent-busy — requires *both* the marker's age to reach 300 seconds *and* the pane's on-screen content to have then been frozen for that same 300-second window, not age alone. Because the freeze window only starts sampling once age first reaches 300s, the **effective combined wait is not 300s but roughly 640s** (measured live across four independent trips: 641s / 626s / 645s / 619s — cmd_230, `queue/reports/gunshi_report.yaml` finding_3). It shares the hash-comparison mechanism `is_stalled_pane()` uses for stall detection (same idea — screen movement means alive — but its own state and its own threshold, never mixed with `is_stalled_pane()`'s `stall_after_sec`). Age alone used to be sufficient to force an agent to idle, which meant a genuinely long-running turn (screen still updating) could trip the exact stuck-busy failure the two-marker design exists to prevent — this happened live to an agent during this feature's own design review. If pane capture fails or returns empty, the safety net treats that as indeterminate and does **not** force idle. This net only engages once at least one message is unread (`FIRST_UNREAD_SEEN > 0`) — with zero unread messages it cannot fire at all (cmd_243 §0-D), which is what the third net below closes.

**cmd_243 — a third, unread-independent net** (`stale_busy_mark_net()` in `scripts/inbox_watcher.sh`, called from `liveness_tick()`) reads the busy marker's own mtime directly instead of proxying through `FIRST_UNREAD_SEEN`, so it fires on the same wall-clock timer `liveness_tick()` already gives stall detection — regardless of whether any message has ever arrived unread. It keeps the same two-part gate as the net above (marker age >= 300s **and** pane hash frozen >= 300s) and additionally checks `pane_input_safety()`: a screen classified `modal` (a real input-waiting prompt) is never forced idle — only notified — so this net cannot cut off a turn that is genuinely waiting on a human. Log lines are tagged `[STALE-BUSY-MARK]` to distinguish them from the legacy net's `stale busy recovery` lines; both remain in place side by side (the legacy net is not removed) and each keeps its own one-shot notify guard. Forcing idle here can pre-empt stall detection's own escalation (which waits the longer `stall_after_sec`, default 900s) by up to ~300s — accepted, since this net's own notify reports the same fact sooner.

The display layer has a parallel fix (cmd_243 §B). `get_pane_busy_rc()` in `lib/agent_status.sh` used to fall through to pane-text analysis (`agent_is_busy_check()`) any time a `claude`-type agent's turn-state marker read idle — including when a busy marker had just been frozen and forced idle by either safety net above. The instant after a stale-busy net fires, the pane still shows its last on-screen status line (often literally containing `esc to interrupt`, since that's what a frozen "still working" screen looks like), so the display kept reporting busy even though the marker was now correctly idle — the fix (the marker and the display it feeds) landed only halfway. `get_pane_busy_rc()` now falls through to pane-text analysis only when no hook-written marker exists at all (`/tmp/shogun_ups_{agent}` and `/tmp/shogun_busy_{agent}` both absent) — a condition related to, but not identical with, the mtime-based `[HOOK-UNARMED]` check above. When either marker exists, an idle turn-state is now trusted as-is.

A matcher-less `PostToolUse` hook (`scripts/post_tool_use_hook.sh`, every tool call) additionally re-touches the busy marker mid-turn — previously nothing advanced either marker between a turn's start and its end, so any turn running past the stale-busy net's ~640s effective window would eventually trip it regardless of the pane-freeze check above. This is a reinforcement of that check, not a replacement for it: the stale-busy net is designed to work correctly even with this hook unarmed. **Registering a new hook in `.claude/settings.json` does not reach an already-running CLI session** — it only takes effect once that session's process is restarted.

Delivery paths that must not fire during a modal-blocked pane still carry their own independent gate ([`pane_has_open_modal()`](lib/agent_status.sh)) — this is a second, orthogonal net that stays useful precisely on days the hook isn't armed.

**Pane-width caveat**: pane-text-based busy detection (`agent_is_busy_check()`, used for non-`claude` CLIs and for stall detection) is unreliable at tmux pane width 44 — busy agents can render as idle because Claude Code's status-bar hint line gets truncated before the `esc to interrupt` substring appears (measured 2026-08-09; affects roughly two-thirds of this repo's default layout). The two-marker design above does not read pane text at all, so it is unaffected; the pane-width fix itself is tracked separately.
...
**🐕 Watchdog & Baton Recovery**

Separate from message delivery, another layer watches for agents that go silent entirely — an agent holding a task and stuck mid-turn looks the same as one that's thinking, so "no unread messages" alone isn't enough:

- **`baton_watchdog.sh`** — a single global instance (not one per agent) that reads `queue/` YAML on a ~60s cycle and checks four independent stall patterns: no one holds any task while work remains open ("baton loss"), the delivery mechanism itself has died, an agent holds a task but hasn't touched it in a long time, or an agent's inbox sits unread despite its watcher being alive. Its stall detection is **read-only diagnosis** — it never sends tmux keystrokes, kills processes, or reassigns work. It only writes a notification (to another agent's inbox, and for longer stalls, to the Lord's phone via ntfy). One exception: the same script also implements an opt-in periodic context reset (`periodic_clear_enabled`) that writes a `clear_command` for `karo`/`gunshi` when they have been idle — delivered by `inbox_watcher.sh` as a `/clear` keystroke. It is disabled by default and requires the Lord's approval to enable. A `cmd` waiting on the Lord can be marked in `queue/shogun_to_karo.yaml` so "baton loss" doesn't nag every ~900s while the Lord is genuinely away from keyboard — the marker only widens the nag interval, it never silences detection: a `baton_lost_human_held_after_sec` safety net (default 86400s/24h) always fires regardless, in case the marker is stale or wrong, timed from the oldest `awaiting_since` among cmds still marked. **Both fields must be written together: `awaiting: lord` and `awaiting_since: "<ISO8601 timestamp>"`.** Writing `awaiting: lord` alone falls back to timing from the moment the watchdog process happens to notice it, which resets across a watchdog restart — the exact bug (OBS-61-1) the safety net exists to prevent.
- **`watcher_supervisor.sh`** — a lightweight loop that keeps every `inbox_watcher.sh` instance and `baton_watchdog.sh` alive, restarting any that die. Duplicate launches are guarded at every level via `pgrep` (the supervisor itself, each watcher, and the watchdog).

**Restarting after a code change:** these are long-running processes — code changing on disk doesn't update a daemon that's already running. Swapping one in requires stopping the old process (never done by an agent itself — see `D006` in `CLAUDE.md`) and letting the supervisor relaunch it, which happens within seconds. This project always verifies a daemon's *start time* against the *merge time* of the code it's supposed to be running, rather than trusting that "merged" means "live."
