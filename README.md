...
- **`baton_watchdog.sh`** — a single global instance (not one per agent) that reads `queue/` YAML on a ~60s cycle and checks four independent stall patterns: no one holds any task while work remains open ("baton loss"), the delivery mechanism itself has died, an agent holds a task but hasn't touched it in a long time, or an agent's inbox sits unread despite its watcher being alive. Its stall detection is **read-only diagnosis** — it never sends tmux keystrokes, kills processes, or reassigns work. It only writes a notification (to another agent's inbox, and for longer stalls, to the Lord's phone via ntfy). One exception: the same script also implements an opt-in periodic context reset (`periodic_clear_enabled`) that writes a `clear_command` for `karo`/`gunshi` when they have been idle — delivered by `inbox_watcher.sh` as a `/clear` keystroke. It is disabled by default and requires the Lord's approval to enable. A `cmd` waiting on the Lord can be marked `awaiting: lord` in `queue/shogun_to_karo.yaml` so "baton loss" doesn't nag every ~900s while the Lord is genuinely away from keyboard — the marker only widens the nag interval, it never silences detection: a `baton_lost_human_held_after_sec` safety net (default 86400s/24h) always fires regardless, in case the marker is stale or wrong.
