...
Phase 2 distinguishes busy vs idle for Claude Code agents with a **two-marker design** (cmd_217): two timestamp files, `/tmp/shogun_busy_{agent}` and `/tmp/shogun_idle_{agent}`, are compared by mtime — `agent_turn_state()` in [`lib/agent_status.sh`](lib/agent_status.sh) reports `busy` when the busy marker's mtime is `>=` the idle marker's (ties go to busy — the safe side), and `idle` otherwise, including when the busy marker doesn't exist yet. This replaced an earlier presence-only design where a single idle flag, once created, was never deleted mid-session — `agent_is_busy()` for `claude` ended up reporting idle almost permanently regardless of actual pane state, because touching the flag's *contents* couldn't change a check that only asked "does it exist." The two-marker design instead reads CLI-reported turn boundaries: the busy marker is touched by the `UserPromptSubmit` hook ([`scripts/user_prompt_submit_hook.sh`](scripts/user_prompt_submit_hook.sh)) when a turn starts, by `session_start_hook.sh` at session start, and by `stop_hook_inbox.sh`'s block path when a turn continues; the idle marker is touched by `stop_hook_inbox.sh`'s normal exit path and by `inbox_watcher.sh` at startup. If the `UserPromptSubmit` hook is never armed (not registered in the running CLI process), the busy marker simply never appears and the judgment quietly degrades to "always idle" — identical to today's de facto behavior, not a regression. `inbox_watcher.sh` self-reports this: after `N=3` nudges with no busy marker ever observed, it logs `[HOOK-UNARMED]` (once) and notifies; if the marker does appear, it logs `[HOOK-ARMED]` (once). A 300-second stale-busy safety net remains the sole upper bound on permanent-busy (the only failure mode that can produce it is a Stop hook that never fires). Delivery paths that must not fire during a modal-blocked pane still carry their own independent gate ([`pane_has_open_modal()`](lib/agent_status.sh)) — this is a second, orthogonal net that stays useful precisely on days the hook isn't armed.
