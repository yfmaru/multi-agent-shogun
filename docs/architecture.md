...
stale-busy網が解く（有界。cmd_243以前は未読0の間この網が原理的に
発火し得ず無界だったが、`stale_busy_mark_net()`が`liveness_tick()`
経由で未読非依存に動くようになったため、現在は未読の有無によらず
有界である）、touch側の誤りは欠陥2の再演で解く者が居らぬ（無界）。
...
（cmd_243以降は未読の有無によらず発火する。上記(c)の注記参照）と
900秒stall通知が実害の上限を与えるため、本cmdの射程外・文書化のみ
とした。
