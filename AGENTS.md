...
2. **Read `memory/MEMORY.md`** — persistent cross-session memory. Codex CLI使用時は全Claudeエージェントのセッションへ自動注入されるため明示的なReadは不要。他CLI（Codex/OpenCode/Kimi等）では自動注入されぬため、将軍のみ明示的にReadせよ。
